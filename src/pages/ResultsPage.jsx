...
export default function ResultsPage() {
  ...
  // Use real analysis if available, otherwise mock
  const analysis = currentAnalysis || mockAnalysis;
  const findings = analysis.findings || [];

  const handleGenerateReport = () => {
    toast.success('Generating radiology report…');
    navigate('/report');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-500 font-mono">
            Model: ClearScan v1.0 · DenseNet-121
          </span>
          <button
            onClick={() => navigate('/')}
      ...
      <motion.div
        ...
        className="flex items-center justify-between gap-4 px-5 py-4 rounded-xl border border-slate-700 bg-slate-800/60"
      >
        <div>
          ...
          <p className="text-sm text-slate-300">
            ...
            {findings.filter((f) => f.confidence > 0.8).length} high-confidence
          </p>
        </div>
        ...
        <div className="lg:col-span-3 flex flex-col gap-4">
          ...
          <div className="space-y-3">
            {[...findings]
              ...
              .map((finding, idx) => (
                <FindingCard key={finding.name} finding={finding} index={idx} />
              ))}
          </div>
          ...
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: findings.length * 0.1 + 0.2 }}
            className="mt-2"
          >
            <button
              onClick={handleGenerateReport}
              className="w-full flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-blue-500 hover:bg-blue-600 text-white font-semibold text-sm transition-colors shadow-glow-blue"
            >
              <FileText className="h-4 w-4" />
              Generate Radiology Report
            </button>
            <p className="text-center text-xs text-slate-500 mt-2">
              AI-drafted report · GPT-4o · RADLEX-compliant structure
            </p>
          </motion.div>
        </div>
      </div>
